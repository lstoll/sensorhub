...
type CollatedMetrics struct {
	...
	gauges                         map[string]chan interface{}
...
func NewCollatedMetrics(user, token, source string,
	...
	m := &CollatedMetrics{
		...
		make(chan map[string]interface{}, collateMax),
		make(map[string]chan int64), make(map[string]chan interface{}),
...
func (m *CollatedMetrics) GetGauge(name string) chan interface{} {
	ch, ok := m.gauges[name]
	if ok {
		return ch
	}
...
func (m *CollatedMetrics) NewGauge(name string) chan interface{} {
